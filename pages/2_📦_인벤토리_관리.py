...
def create_template_excel():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # CAS No 컬럼(F열, 6번째)을 텍스트 형식으로 설정
        for row in range(1, len(df) + 1):
            cell = worksheet.cell(row=row, column=6)  # F열 = CAS No
    ...
    try:
        ...
        if result.get('success'):
            ...
            return {
                ...
                '중점관리물질': classifications.get('중점관리물질', '-'),
                '인체유해성물질': classifications.get('인체유해성물질', '-'),
    ...
    if st.button("🔍 API 일괄 조회 시작", type="primary", use_container_width=True, disabled=api_disabled):
        ...
        for idx, row in df_result.iterrows():
            ...
            if cas_no in results:
                ...
                keco = res.get('keco', {})
                if keco:
                    # 화관법 규제정보 (O 또는 O(함량%) 형식)
                    기존 = keco.get('기존화학물질', '-')
                    df_result.at[idx, '기존'] = 기존 if 기존.startswith('O') else '-'
                    
                    # 사고대비물질 (함량 포함: O(85%이상))
                    df_result.at[idx, '사고대비'] = keco.get('사고대비물질', '-')
                    
                    # 등록대상기존화학물질
                    등록대상 = keco.get('등록대상기존화학물질', '-')
                    df_result.at[idx, '등록대상기존화학물질'] = 등록대상 if 등록대상.startswith('O') else '-'
                    
                    # 인체유해성물질 → 급성·만성·생태 컬럼에 표시 (함량 포함)
                    인체유해 = keco.get('인체유해성물질', '-')
                    if 인체유해.startswith('O'):
                        df_result.at[idx, '급성·만성·생태'] = 인체유해
                    
                    # 중점관리물질 (CMR 정보) → 중점 컬럼에 추가
                    중점 = keco.get('중점관리물질', '-')
                    if 중점.startswith('O'):
                        existing = str(df_result.at[idx, '중점']) if pd.notna(df_result.at[idx, '중점']) else ''
                        if existing and 'PRTR' in existing:
                            df_result.at[idx, '중점'] = f"{existing}/{중점}"
                        else:
                            df_result.at[idx, '중점'] = 중점
                    ...
                    if 제한.startswith('O'):
                        restrictions.append(f"제한{제한[1:]}" if len(제한) > 1 else '제한')
                    if 금지.startswith('O'):
                        restrictions.append(f"금지{금지[1:]}" if len(금지) > 1 else '금지')
                    if 허가.startswith('O'):
                        restrictions.append(f"허가{허가[1:]}" if len(허가) > 1 else '허가')
...
if st.session_state.processed_data is not None:
    ...
    with col4:
        cnt = len(df_result[df_result['사고대비'].notna() & df_result['사고대비'].astype(str).str.startswith('O')])
    ...
    with col1:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # CAS No 컬럼을 텍스트 형식으로 설정 (날짜 변환 방지)
            ws = writer.sheets['화학물질 정보']
            from openpyxl.styles import numbers
            
            # CAS No 컬럼 인덱스 찾기
            cas_col_idx = None
            for col_idx, col_name in enumerate(df_result.columns, 1):
                if 'CAS' in str(col_name).upper():
                    cas_col_idx = col_idx
                    break
            
            # CAS No 컬럼 전체를 텍스트 형식으로 설정
            if cas_col_idx:
                for row in range(2, len(df_result) + 2):  # 헤더 다음 행부터
                    cell = ws.cell(row=row, column=cas_col_idx)
                    cell.number_format = '@'  # 텍스트 형식
                    # 값이 있으면 문자열로 다시 설정
            ...
            summary_data = {
                ...
                '건수': [
                    ...
                    len(df_result[df_result['사고대비'].notna() & df_result['사고대비'].astype(str).str.startswith('O')]),
