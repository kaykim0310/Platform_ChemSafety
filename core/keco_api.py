...
def extract_percent_from_text(text: str) -> str:
    ...
    - "1% 이상 함유한 혼합물" → "1%이상"
    - "25% 미만" → "25%미만"
    - "인체급성유해성 : 1%, 인체만성유해성 : 0.1%" → "급성1%/만성0.1%"
    """
    import re
    ...
    # 특수 케이스: 인체급성/만성유해성
    if "급성" in text and "만성" in text:
        acute = re.search(r'급성[^:]*:\s*(\d+(?:\.\d+)?)\s*%', text)
        chronic = re.search(r'만성[^:]*:\s*(\d+(?:\.\d+)?)\s*%', text)
        parts = []
        if acute:
            parts.append(f"급성{acute.group(1)}%")
        if chronic:
            parts.append(f"만성{chronic.group(1)}%")
        if parts:
            return "/".join(parts)
    
    # 일반 패턴: 숫자% + 이상/이하/초과/미만
    patterns = [
...
def extract_classifications(type_list: List[Dict]) -> Dict[str, str]:
    ...
    [
        ...
        {"sbstnClsfTypeNm": "인체등유해성물질", "contInfo": "인체급성유해성 : 1%, 인체만성유해성 : 0.1%", ...},
    ]
    ...
    for item in type_list:
        ...
        # "인체등유해성물질" → "인체유해성물질"로 매핑
        if type_name == "인체등유해성물질":
            type_name = "인체유해성물질"
        ...
        if type_name in classification_map:
            ...
            # 중점관리물질의 경우 excpInfo에 CMR 정보가 있음
            if type_name == "중점관리물질" and excp_info:
                cmr_info = excp_info.strip()
                if cmr_info:
                    classification_map[type_name] = f"O({cmr_info})"
