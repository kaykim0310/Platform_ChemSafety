...
if st.session_state.inventory_data is not None:
    ...
    with col1:
        if st.button("🔍 KOSHA API 일괄 조회 시작", type="primary", width="stretch", disabled=not KOSHA_AVAILABLE):
            ...
            for idx, cas_no in enumerate(unique_cas):
                elapsed = time_module.time() - start_time
                remaining = (elapsed / (idx + 1)) * (total_unique - idx - 1) if idx > 0 else 0
                
                status_text.text(f"조회 중... {idx+1}/{total_unique} - {cas_no}")
                time_text.text(f"⏱️ 경과: {elapsed:.0f}초 | 예상 남은 시간: {remaining:.0f}초")
                progress_bar.progress((idx + 1) / total_unique)
