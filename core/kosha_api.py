...
def get_legal_regulations(chem_id: str) -> Dict[str, str]:
    ...
    for item in items:
        ...
        # ====== 위험물안전관리법 ======
        if "위험물" in name_kor and "안전관리법" in name_kor:
            import re
            
            # 예: "4류 제1석유류(비수용성) (200L)"
            # 예: "제4류 알코올류 (400L)"
            ...
            # 품명 추출 - 류별 뒤의 텍스트에서 괄호 전까지
            # "4류 제1석유류(비수용성) (200L)" → "제1석유류(비수용성)"
            name_match = re.search(r'[1-6]류\s+(.+?)\s*\(\d+', detail)
            if name_match:
                result["hazmatName"] = name_match.group(1).strip()
            else:
                # 백업: 알려진 품명 키워드로 추출
                hazmat_names = [
            ...
            # 지정수량 추출 - 괄호 안의 숫자+단위
            qty_match = re.search(r'\((\d+)\s*(L|ℓ|kg)\)', detail, re.IGNORECASE)
            if qty_match:
                qty_val = qty_match.group(1)
                qty_unit = qty_match.group(2).upper().replace("ℓ", "L")
                result["hazmatQty"] = f"{qty_val}{qty_unit}"
            
            # 위험등급 - 품명 기준 정확한 매핑
            hazmat_name = result.get("hazmatName", "")
            hazmat_class = result.get("hazmatClass", "")
            
            # 위험등급 I (지정수량 50L 이하)
            grade_1 = ["특수인화물"]
            
            # 위험등급 II (지정수량 50L 초과 ~ 400L 이하)
            grade_2 = ["제1석유류", "알코올류", "산화성고체", "황화인", "황린", 
                       "적린", "유황", "철분", "금속분", "마그네슘", "인화성고체",
                       "칼륨", "나트륨", "알킬알루미늄", "자연발화성", "금수성물질"]
            
            # 위험등급 III (지정수량 400L 초과)
            grade_3 = ["제2석유류", "제3석유류", "제4석유류", "동식물유류",
                       "니트로화합물", "유기과산화물", "염소산염류", "과염소산염류",
                       "무기과산화물", "아염소산염류", "브롬산염류", "질산염류",
                       "요오드산염류", "과망간산염류", "중크롬산염류"]
            
            # 품명으로 위험등급 결정
            if any(g in hazmat_name for g in grade_1):
                result["hazmatGrade"] = "I"
            elif any(g in hazmat_name for g in grade_2):
                result["hazmatGrade"] = "II"
            elif any(g in hazmat_name for g in grade_3):
                result["hazmatGrade"] = "III"
            else:
                # 류별로 기본값 설정
                if hazmat_class == "제1류":
                    result["hazmatGrade"] = "I"
                elif hazmat_class in ["제2류", "제3류"]:
                    result["hazmatGrade"] = "II"
                elif hazmat_class in ["제4류", "제5류", "제6류"]:
                    result["hazmatGrade"] = "II"  # 제4류는 품명에 따라 다름
        
        # ====== 화학물질관리법 / 화관법 / 환경부 ======
        if any(k in name_kor for k in ["화학물질관리법", "화관법", "유해화학물질", "환경부", "화학물질의 등록"]):
